...
with tab1:
    ...
    if st.button("🔍 Analyse", type="primary", use_container_width=True):
        if not user_text.strip():
            ...
            # Result card
            colour_map = {"positive": "#2ecc71", "negative": "#e74c3c", "neutral": "#95a5a6"}
            ...
            fig_bar.update_layout(
                height=200, margin=dict(l=0, r=60, t=10, b=10),
                showlegend=False, template=chart_theme,
...
with tab2:
    ...
    results_df = pd.DataFrame([
        ...
        {"Model": "TF-IDF + SVM",          "Accuracy": 67.24, "Neg F1": 0.14, "Neu F1": 0.75, "Pos F1": 0.71, "Training": "< 1 min"},
        {"Model": "FinBERT (ProsusAI)",    "Accuracy": 91.40, "Neg F1": 0.88, "Neu F1": 0.92, "Pos F1": 0.94, "Training": "Pre-trained"},
    ])

    # Accuracy bar chart
    colours = ["#e74c3c" if a < 50 else "#f39c12" if a < 75 else "#2ecc71"
    ...
    fig_acc.add_hline(y=53.6, line_dash="dot", line_color="gray",
    ...
    # F1 per class
    st.markdown("**Per-class F1 scores** — the real measure of quality on imbalanced data:")
    fig_f1 = go.Figure()
    for cls, colour in [("Neg F1", "#e74c3c"), ("Neu F1", "#95a5a6"), ("Pos F1", "#2ecc71")]:
    ...
    st.caption(
        "⚠️ Classical models struggle with the negative class (only 14.7% of data). "
        "FinBERT uses contextual attention to understand negation and domain nuance."
    )
...
with tab3:
    ...
    c3.metric("Direction acc", f"{dir_acc*100:.1f}%", delta=f"{(dir_acc-0.5)*100:+.1f}% vs random")
    ...
    days = list(range(n))
    fig_ts.add_trace(
        go.Scatter(x=days, y=x, fill="tozeroy",
                   line=dict(color="#3498db"), name="Sentiment"),
        row=1, col=1,
    )
    ...
    fig_ts.update_layout(
        template=chart_theme, height=500, showlegend=True,
        margin=dict(t=40),
    )
    ...
    st.info(
        "📌 **Interpretation:** Even small correlations (r=0.1–0.2) are valuable in "
        "finance. A system with 53% directional accuracy, applied consistently "
        "across thousands of trades, generates significant alpha over time "
        "(this is the basis of systematic hedge funds)."
    )
...
with tab4:
    ...
    search_term = st.text_input("Search for a keyword", placeholder="e.g. profit, acquisition")

    filtered = df[df["Sentiment"].isin(filter_label)]
    if search_term:
        filtered = filtered[filtered["Sentence"].str.lower().str.contains(
            search_term.lower(), na=False
        )]

    st.caption(f"Showing {len(filtered):,} of {len(df):,} sentences")
    st.dataframe(
        filtered[["Sentence", "Sentiment"]].rename(
            columns={"Sentence": "Headline", "Sentiment": "Label"}
        ).head(200),
